...
export default async function CreateProfile() {
  ...
  let health;
  {
    const result = await db.query(
      `SELECT health FROM user_info WHERE user_name = $1`,
      [name]
    );
    console.log(result);
  ...
  console.log('this should be a number', health);

  let damage;
  {
    const result = await db.query(
      `SELECT dps FROM user_info WHERE user_name = $1`,
      [name]
    );
    console.log(result);

    damage = result.rows[0].dps;
  }
  console.log('this should be a number', damage);
  ...
  async function handleSave(formData) {
    ...
    await db.query(
      `INSERT INTO user_info (user_id, user_name, characternumber, user_bio) VALUES ($1, $2, $3, $4) ON CONFLICT (user_id) DO UPDATE SET user_bio = EXCLUDED.user_bio, characternumber = EXCLUDED.characternumber`,
      [id, name, avatar, bio]
    );
    ...
    <div className='min-h-screen flex flex-col items-center bg-gray-900 text-white py-10 px-4'>
      ...
      <div className='bg-gray-800 p-6 rounded-lg shadow-md w-full max-w-md'>
        ...
        <form className='space-y-4' action={handleSave}>
          <div>
            <label htmlFor='bio' className='text-yellow-400 font-semibold'>
              Bio:
            </label>
            <textarea
              name='bio'
