...
export default async function Profile() {
  ...
  return (
    <div className='min-h-screen bg-gray-900 text-white flex flex-col items-center py-10 px-4'>
      <h1 className='text-4xl font-extrabold mb-8 text-transparent bg-clip-text bg-gradient-to-r from-purple-500 via-pink-500 to-red-500'>
        Profile
      </h1>
      ...
      {uiw.map((item) => (
        <div
          key={item.user_name}
          className='bg-gray-800 p-8 rounded-lg shadow-lg w-full max-w-md hover:shadow-yellow-400/50 transition duration-300 transform hover:scale-95'
        >
          <p className='text-lg font-semibold text-yellow-400 mb-2'>
            Username: <span className='text-white'>{item.user_name}</span>
          </p>
          ...
          <div className='space-y-2'>
            <p className='text-lg font-semibold text-yellow-400'>
              Health: <span className='text-white'>{item.health}</span>
            </p>
            <p className='text-lg font-semibold text-yellow-400'>
              Damage: <span className='text-white'>{item.dps}</span>
            </p>
            <p className='text-lg font-semibold text-yellow-400'>
              Counter: <span className='text-white'>{item.counter}</span>
            </p>
          </div>
          ...
          <div className='mt-6'>
            ...
            <div className='table w-full border-separate border-spacing-y-2'>
              {[
                ...
                >
                  <div className='table-cell py-2 px-4 text-white text-center border border-yellow-400'>
                    {gear}
                  </div>
                </div>
              ))}
